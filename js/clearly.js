...
window.__getReqianduanClearlyResults = function () {
    ...
    // debug
    ReqianduanClearly.log = function (message) {
    ...
    // text length,
    ReqianduanClearly.getTextLength = function (text) {
        return text.replace(/[\s\n\r]+/gi, '').length;
    };

    // word count,
    ReqianduanClearly.getWordCount = function (text) {
        var _text = text;
        // do stuff
    ...
    // content
    ReqianduanClearly.getContent = function () {
        this.detectLanguage();
        return this.find();
    };

    // Detect language by randomly selected paragraphs from page
    ReqianduanClearly.detectLanguage = function () {
    ...
    ReqianduanClearly.getAbsoluteUrl = function (url) {
    ...
    // _return__links.push({href: ReqianduanClearly.getAbsoluteUrl(_href), text: _node.textContent});
    // _return__images.push({type: 'large', src: ReqianduanClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});
    // _return__images.push({type: 'medium', src: ReqianduanClearly.getAbsoluteUrl(_node.src), width: _width, height: _height});

    // Decide tagName of a node, we just need #text, a, img nodes
    ReqianduanClearly.getContent_detectNodeTagName = function (_node) {
        var _tag_name;
    ...
    ReqianduanClearly.exploreNodeAndGetStuff = function (_nodeToExplore, _justExploring) {
        ...
        var _recursive = function (_node) {
            ...
            var _tag_name = ReqianduanClearly.getContent_detectNodeTagName(_node),
                _result = {
            ...
            case ((_tag_name === '#text')):
                ...
                // result
                _result._length__plain_text = ReqianduanClearly.getTextLength(_nodeText);
                _result._count__plain_words = ReqianduanClearly.getWordCount(_nodeText);
            ...
            if (_result._is__container || ((_result.__index === 1) && (_justExploring === true))) {
                ...
                if (!_justExploring) {
                    switch (true) {
                    case ((ReqianduanClearly.language !== 'cjk') && ((_result._count__links * 2) >= _result._count__plain_words)):
                        /* link ratio */
                    case ((ReqianduanClearly.language !== 'cjk') && (_result._length__plain_text < (65 / 3))):
                        /* text length */
                    case ((ReqianduanClearly.language !== 'cjk') && (_result._count__plain_words < 5)):
                        /* words */
                    case ((ReqianduanClearly.language === 'cjk') && (_result._length__plain_text < 10)):
                        /* text length */
                    case ((ReqianduanClearly.language === 'cjk') && (_result._count__plain_words < 2)):
                        /* words */
                        break;
                    default:
                        // good candidate
    ...
    ReqianduanClearly.processCandidates = function (_candidatesToProcess) {
        // process this var
        ...
        for (var i = 0, _i = _candidates.length; i < _i; i++) {
            ...
            // candidate details
            _candidates[i]['__candidate_details'] = ReqianduanClearly.computeDetailsForCandidate(_candidates[i], _main);
            ...
            // points
            _candidates[i].__points_history = ReqianduanClearly.computePointsForCandidate(_candidates[i], _main);
    ...
    ReqianduanClearly.computeDetailsForCandidate = function (_e, _main) {
        var _r = {};
    ...
    ReqianduanClearly.computePointsForCandidate = function (_e, _main) {
    ...
    ReqianduanClearly.processCandidatesSecond = function (_processedCandidates) {
    ...
    ReqianduanClearly.computeDetailsForCandidateSecond = function (_e, _main) {
        var _r = {};
    ...
    ReqianduanClearly.computePointsForCandidateSecond = function (_e) {
        var _details = _e.__candidate_details,
    ...
    ReqianduanClearly.computePointsForCandidateThird = function (_e) {
        var _details = _e.__candidate_details,
            _details_second = _e.__candidate_details_second,
            _points_history = [];
    ...
    ReqianduanClearly.computePointsForCandidate__do = function (_ratio_remaining, _power, _ratio, _points_history) {
    ...
    ReqianduanClearly.buildHTMLForNode = function (_nodeToBuildHTMLFor, _custom_mode) {
        var _global__element_index = 0,
            _global__the_html = '',
        ...
        var _recursive = function (_node) {
            ...
            // vars
            var _explored = false,
                _tag_name = ReqianduanClearly.getContent_detectNodeTagName(_node),
            ...
            case ('iframe'):
                ...
                ReqianduanClearly.log('Video SRC: ' + _src);
            ...
            // skipped link
            if (_tag_name === 'a') {
                _explored = (_explored || ReqianduanClearly.exploreNodeAndGetStuff(_node, true));
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (ReqianduanClearly.language === 'cjk' && (_explored._length__plain_text > (65 * 3 * 1))):
            ...
            if (_tag_name === 'img') {
                ...
                case (_explored._is__image_large):
                    ReqianduanClearly.images.push({src: ReqianduanClearly.getAbsoluteUrl(_node.src), width: _$node.width(), height: _$node.height()});
            ...
            if (_tag_name === 'a') {
                _explored = (_explored || ReqianduanClearly.exploreNodeAndGetStuff(_node, true));
                _$node = $(_node);
                ReqianduanClearly.links.push({href: ReqianduanClearly.getAbsoluteUrl(_node.href), text: _$node.text()});
                switch (true) {
            ...
            default:
                ...
                _contentsLength = ReqianduanClearly.getTextLength(_contents);
                switch (true) {
            ...
            if (parsingOptions._elements_link_density.indexOf('|' + _tag_name + '|') > -1) {
                ...
                case (!((ReqianduanClearly.language === 'cjk') && (_contentsLength / _initialLength) < 0.1)):
                case ((_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.25))):
                case ((ReqianduanClearly.language === 'cjk') && (_global__exploreNodeToBuildHTMLFor && ((_explored._length__plain_text / _global__exploreNodeToBuildHTMLFor._length__plain_text) > 0.1))):
                    break;
                default:
                    _global__the_html = _global__the_html.substr(0, _pos__start__before);
                    return;
    ...
    ReqianduanClearly.find = function () {
        // get content
        ...
        (function () {
            while (true) {
                ...
                if (_prevNode.previousSibling) {
                    ...
                    // get html
                    var _h = ReqianduanClearly.buildHTMLForNode(_prevNode, 'above-the-target');
                    _prevHTML = _h + _prevHTML;
                    _foundHTML = _h + _foundHTML;

                    // finished?
                    if (ReqianduanClearly.getTextLength(_prevHTML.replace(/<[^>]+?>/gi, '')) > (65 * 3 * 3)) {
                        return;
                    }
                    ...
                    if (_headingStartPos > -1) {
                        var _toHeadingLength = ReqianduanClearly.getTextLength(_foundHTML.substr(0, _headingStartPos).replace(/<[^>]+?>/gi, ''));
                        if (_toHeadingLength < (65 * 3 * 2)) {
                            ReqianduanClearly.log('use title found in page');
                            _foundTitle = true;
        ...
        if (!_foundTitle) {
            if (document.title > '') {
                ReqianduanClearly.log('use title from document.title');
                var _the_title = '',
    ...
    ReqianduanClearly.findInPage = function (_pageWindow) {
    ...
    // auto-launch
    return ReqianduanClearly.getContent();
